 ...
 * в ORM модели, для таблицы shop_items
 */
class App_Model_Mapper_ShopItem
{
    ...
    public function getDbTable()
    {
        if (!$this->_dbTable instanceof Zend_Db_Table_Abstract) {
            $this->setDbTable('App_Model_DbTable_ShopItem');
        }
     ...
     *
     * @param App_Model_ShopItem $entity Объект сущности
     * @return integer|boolean
     */
    public function save(App_Model_ShopItem $entity)
    {
        $data = array();
        ...
        if ($entity->getChips() !== null) {
            $data['chips'] = $entity->getChips();
        }
        if ($entity->getMoney() !== null) {
            $data['money'] = $entity->getMoney();
        }
        if ($entity->getItemCount() !== null) {
            $data['item_count'] = $entity->getItemCount();
        }
        if ($entity->getLifetime() !== null) {
            $data['lifetime'] = $entity->getLifetime();
        }
        if (!isset($data['id'])) {
     ...
     */
    public function find(App_Model_ShopItem $entity, $id)
    {
        ...
        $entity->setChips($row->chips);
        $entity->setMoney($row->money);
        $entity->setItemCount($row->item_count);
        $entity->setLifetime($row->lifetime);
    }
     ...
     */
    public function fetchRow(App_Model_ShopItem $entity, $where = null, $order = null)
    {
