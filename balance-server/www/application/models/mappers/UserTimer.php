 ...
 * в ORM модели, для таблицы user_timers
 */
class App_Model_Mapper_UserTimer
{
    ...
    public function getDbTable()
    {
        if (!$this->_dbTable instanceof Zend_Db_Table_Abstract) {
            $this->setDbTable('App_Model_DbTable_UserTimer');
        }
     ...
     *
     * @param App_Model_UserTimer $entity Объект сущности
     * @return integer|boolean
     */
    public function save(App_Model_UserTimer $entity)
    {
        $data = array();
        ...
        if ($entity->getHourBonus() !== null) {
            $data['hour_bonus'] = $entity->getHourBonus();
        }
        if ($entity->getDailyBonus() !== null) {
            $data['daily_bonus'] = $entity->getDailyBonus();
        }
        if ($entity->getFriendPresent() !== null) {
            $data['friend_present'] = $entity->getFriendPresent();
        }
        if (!isset($data['id'])) {
     ...
     */
    public function find(App_Model_UserTimer $entity, $id)
    {
        ...
        $entity->setHourBonus($row->hour_bonus);
        $entity->setDailyBonus($row->daily_bonus);
        $entity->setFriendPresent($row->friend_present);
    }
     ...
     */
    public function fetchRow(App_Model_UserTimer $entity, $where = null, $order = null)
    {
    ...
    public function fetchAll($where = null, $order = null, $count = null, $offset = null)
    {
        ...
        if ($result->count() > 0) {
            foreach($result as $row) {
                $entity = new App_Model_UserTimer();
                $entity->setId($row->id);
                $entity->setIdUser($row->id_user);
                $entity->setHourBonus($row->hour_bonus);
                $entity->setDailyBonus($row->daily_bonus);
